#!/usr/bin/env python3
"""
Advanced Local Network Chat Room Server - HTTP/WebSocket Version
...
<body>
    <div class="container">
        <div class="header">
            <h1>🔥 Advanced Chat Room</h1>
            <p>Kết nối, chia sẻ và trò chuyện cùng bạn bè</p>
        </div>
        ...
        function addMessage(data) {
            ...
            if (data.type === 'system') {
                ...
                messageDiv.innerHTML = `
                    ...
                    <div class="message-time">${timestamp}</div>
                    ${isOwn && data.reads ? `<div class="read-status">${data.reads} người đã xem</div>` : ''}
                `;
            }
...
def main():
    ...
    print("="*70)
    print("🔥 ADVANCED LOCAL NETWORK CHAT ROOM SERVER")
    print("="*70)
    ...
    log_event(f"Advanced chat server started on {host}:{port}")
